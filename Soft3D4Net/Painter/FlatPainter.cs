...
namespace Soft3D4Net {
    ...
    public class FlatPainter : IPainter {
        ...
        public void DrawTriangle(FrameBuffer surface, ColorRGB color, VertexBuffer vbx, int triangleIndice) {
            ...
            var vertexNormal = (v0.Norm + v1.Norm + v2.Norm) / 3;
            var vertexCenter = (v0.World + v1.World + v2.World) / 3;
            ...
            if(PainterUtils.Cross2D(p0, p1, p2) > 0) {
                ...
                //   P1
                // P2
                paintHalfTriangle(surface, yStart, (int)yMiddle - 1, color, p0, p2, p0, p1);
                paintHalfTriangle(surface, (int)yMiddle, yEnd, color, p0, p2, p1, p2);
            }
            else {
                //   P0
                // P1
                //   P2
                paintHalfTriangle(surface, yStart, (int)yMiddle - 1, color, p0, p1, p0, p2);
                paintHalfTriangle(surface, (int)yMiddle, yEnd, color, p1, p2, p0, p2);
            }
        }

        
        void paintHalfTriangle(FrameBuffer surface, int yStart, int yEnd, ColorRGB color, Vector3 pa, Vector3 pb, Vector3 pc, Vector3 pd) {
        ...
        void ProcessScanLine(FrameBuffer surface, float y, float sx, float ex, float sz, float ez, ColorRGB color) {
